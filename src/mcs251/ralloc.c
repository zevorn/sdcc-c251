/*------------------------------------------------------------------------
  ...
  ralloc.c - source file for MCS251 register allocation
...
extern void mcs251_genCode (iCode *);
#define D(x)
...
spillLRWithPtrReg (symbol * forSym)
{
  ...
  r0 = mcs251_regWithIdx (R0_IDX);
  r1 = mcs251_regWithIdx (R1_IDX);
...
spillThis (symbol * sym)
{
  ...
  /* if spilt on stack then free up r0 & r1
     ...
     LIVE ranges */
  if (!mcs251_ptrRegReq && isSpiltOnStack (sym))
    {
...
spilSomething (iCode * ic, eBBlock * ebp, symbol * forSym)
{
  ...
  if (!mcs251_ptrRegReq && isSpiltOnStack (ssym))
    {
...
updateRegUsage (iCode * ic)
{
  ...
  for (reg = 0; reg < mcs251_nRegs; reg++)
    {
      if (mcs251_regs[reg].isFree)
        {
...
findAllBitregs (void)
{
  bitVect *rmask = newBitVect (mcs251_nRegs);
  int j;

  for (j = 0; j < mcs251_nRegs; j++)
    {
      if (mcs251_regs[j].type == REG_BIT)
        rmask = bitVectSetBit (rmask, mcs251_regs[j].rIdx);
    }
...
/*-----------------------------------------------------------------*/
/* mcs251_allBitregs :- returns bit vector of all bit registers     */
/*-----------------------------------------------------------------*/
bitVect *
...
findAllBankregs (void)
{
  ...
  for (j = 0; j < mcs251_nRegs; j++)
    {
      if ((mcs251_regs[j].type == REG_GPR) || (mcs251_regs[j].type == REG_PTR))
        rmask = bitVectSetBit (rmask, mcs251_regs[j].rIdx);
    }
...
/*-----------------------------------------------------------------*/
/* mcs251_allBankregs :- returns bit vector of all bank registers   */
/*-----------------------------------------------------------------*/
bitVect *
...
regsUsedIniCode (iCode * ic)
{
  ...
  if (IC_LEFT (ic))
    rmask = bitVectUnion (rmask, mcs251_rUmaskForOp (IC_LEFT (ic)));

  if (IC_RIGHT (ic))
    rmask = bitVectUnion (rmask, mcs251_rUmaskForOp (IC_RIGHT (ic)));

  if (IC_RESULT (ic))
    rmask = bitVectUnion (rmask, mcs251_rUmaskForOp (IC_RESULT (ic)));
      ...
      for (ic = ebbs[i]->sch; ic; ic = ic->next)
        {
          ...
          ic->rMask = newBitVect (mcs251_nRegs + 1);
