/*-------------------------------------------------------------------------
  gen.h - header file for code generation for MCS251
...
typedef struct asmop
{
  ...
  aopu;
  signed char regs[END_IDX];   /* Byte held in each tracked register, or -1. */
  struct valinfo valinfo;
}
asmop;

void mcs251_genCode (iCode *);
void mcs251_emitDebuggerSymbol (const char *);

bool mcs251IsReturned (const char *what);
...
bool mcs251IsRegArg (struct sym_link *ftype, int i, const char *what);

// Check if what is part of the any argument (counting from 1) to a function of type ftype.
bool mcs251IsParmInCall(sym_link *ftype, const char *what);

extern const char *mcs251_fReturnRegisters[];
extern unsigned mcs251_fReturnSize;
