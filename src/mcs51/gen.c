...
#define RUNTIME_CALL (false ? "ecall" : "lcall")
#define LONG_JUMP (false ? "ejmp" : "ljmp")
#define LOOP_ENTRY_JUMP (false ? "ejmp" : "sjmp")
...
mcs251LoadFarSymbol (asmop *aop, int offset)
{
  ...
  wassert (false && aop->type == AOP_DPTR && aop->aopu.aop_sym);
  if (offset)
    {
...
aopForSym (iCode * ic, symbol * sym, bool result)
{
  ...
  if (false && sym->onStack && !space->paged)
    {
...
aopForRemat (symbol *sym)
{
  ...
  aop->aopu.aop_immd.mcs251_pdata_page =
    false && from_type && IS_PTR (from_type) &&
...
{
  if (false && !aop->aop_is_aggregate && aop->size > 1)
...
{
  if (false && size > 1 && !IS_AGGREGATE (type))
  ...
  else
    {
      ...
      switch (aop->type)
        ...
        case AOP_DPTR:
          ...
          if (false && aop->aopu.aop_sym)
            {
              mcs251LoadFarSymbol (aop, physicalOffset);
              loadFarPointerByte (aop->code);
            }
          else if (!false && aop->code && aop->coff == 0 && offset >= 1)
            {
                       ...
                       int soffset, int size)
{
  ...
  wassert (false && result->type == AOP_DPTR);
...
pushbigreturn (operand *result)
{
  ...
  if (false && sym->onStack && !options.useXstack)
    {
...
genCall (iCode * ic)
{
  ...
  const char *call = false ? (noreturn ? "ejmp" : "ecall") :
                                     (noreturn ? "ljmp" : "lcall");
  ...
  /* make the call */
  if (IFFUNC_ISBANKEDCALL (dtype) && !false)
    {
  ...
  else
    {
      if (IS_LITERAL (etype))
        {
          emitcode (call, false ? "0x%06X" : "0x%04X",
  ...
  else
    {
      if (IFFUNC_ISBANKEDCALL (dtype) && !false)
      ...
      else                      /* the send set is empty */
        {
          ...
          /* make the call */
          emitcode (call, false ? "@dr28" : "__sdcc_call_dptr");
        }
    }
  ...
  if (IFFUNC_ISISR (ftype))
    {
      ...
      if (!false && currFunc->funcRestartAtomicSupport)
        emitcode (false ? "ejmp" : (options.acall_ajmp ? "ajmp" : "ljmp"),
  ...
  else
    {
      ...
      if (IFFUNC_ISBANKEDCALL (ftype) && !false)
        {
          emitcode (LONG_JUMP, "__sdcc_banked_ret");
        }
      else
        {
          emitcode (false ? "eret" : "ret", "");
        }
    }
  ...
  if (bigreturn)
    {
      ...
      if (!options.useXstack)
        offset -= (false ? 3 : 2) + IFFUNC_ISBANKEDCALL (currFunc->type);
      offset -= framepointer;
      if (!framepointer)
...
genPlusIncr (iCode * ic)
{
  ...
  /* we can if the aops of the left & result match or
     ...
     same */
  if (sameRegs (AOP (IC_LEFT (ic)), AOP (IC_RESULT (ic))) &&
      !(false && AOP_TYPE (IC_LEFT (ic)) == AOP_MCS251_STK))
    {
      const char *l = opGet (IC_LEFT (ic), 0, FALSE, FALSE);
      bool mcs251_step_operand = false &&
  ...
  if (icount == 1 || (false && (icount == 2 || icount == 4)))
    {
...
mcs251DwordForNativeTuple (const asmop *aop)
{
  ...
  if (!false || aop->type != AOP_REG || aop->size != 4)
    return NULL;
...
genCmp (operand * left, operand * right, operand * result, iCode * ifx, int sign, iCode * ic)
{
  ...
  else if (!false && (opIsGptr (left) || opIsGptr (right)))
    {
      /* push right */
...
gencjneshort (operand * left, operand * right, symbol * lbl)
{
  ...
  /* generic pointers require special handling since all NULL pointers must compare equal */
  if (!false && (opIsGptr (left) || opIsGptr (right)))
    {
      /* push right */
      while (offset < size)
        {
                          ...
                          const char *instruction)
{
  ...
  if (!false || !size || AOP_TYPE (result) == AOP_CRY ||
...
genAnd (iCode * ic, iCode * ifx)
{
  ...
  if (false && AOP_TYPE (right) != AOP_REG &&
...
mcs251WriteBackPostincrementedPointer (operand *pointer)
{
  ...
  wassert (false && AOP_TYPE (pointer) == AOP_DPTR);
...
genFarPointerGet (operand * left, operand * result, iCode * ic, iCode * pi, iCode * ifx)
{
  ...
  aopOp (result, ic, FALSE);
  mcs251FarResult = false && AOP_TYPE (result) == AOP_DPTR;
  if (mcs251FarResult)
    {
  ...
  if (pi && AOP_TYPE (left) != AOP_IMMD && AOP_TYPE (left) != AOP_STR)
    {
      if (false && AOP_TYPE (left) == AOP_DPTR)
...
genDataPointerSet (operand * right, operand * result, iCode * ic)
{
  ...
  if (!false && (size > 1) && IS_OP_LITERAL (right))
    {
...
genFarPointerSet (operand * right, operand * result, iCode * ic, iCode * pi)
{
  ...
  /* so dptr now contains the address */
  aopOp (right, ic, FALSE);
  mcs251FarSource = false && AOP_TYPE (right) == AOP_DPTR;
  if (mcs251FarSource)
    {
  ...
  if (pi && AOP_TYPE (result) != AOP_STR && AOP_TYPE (result) != AOP_IMMD)
    {
      if (false && AOP_TYPE (result) == AOP_DPTR)
...
genAddrOf (iCode * ic)
{
  ...
  if (sym->onStack)
    {
      if (false && !SPEC_OCLS (sym->etype)->paged)
        {
          size = AOP_SIZE (IC_RESULT (ic));
  ...
  if (!false && opIsGptr (IC_RESULT (ic)))
    {
      struct dbuf_s dbuf;
...
genAssign (iCode * ic)
{
  ...
  else if (false && AOP_TYPE (result) == AOP_DPTR &&
...
genJumpTab (iCode * ic)
{
  ...
  if ((count <= 7) ||
  ...
  else
    {
      ...
      jtabext = false ? newiTempLabel (NULL) : NULL;
