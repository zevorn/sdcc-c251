...
MCS-251 ABI revision 1 沿用 SDCC MCS-51 的 little-endian object layout，而 CPU
原生 WR/DR memory interpretation 不一定相同。因此只有在 byte ordering 等价，或
显式插入转换时，才可用原生 word/dword instruction 替代逐字节序列。
