...
并实现了可用于常量表达式的类型兼容性查询，但 common frontend 现有的 GNU 子集
仍不足以编译 Zephyr。
...
- 已在 GNU11/GNU17 实现的 `__builtin_types_compatible_p`，以及仍待实现的
  `__builtin_expect`、`__builtin_unreachable`、bit-counting builtin 和 overflow
  builtin；
