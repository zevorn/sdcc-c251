...
| return frame / `setjmp` | 所有 C call 按 3 B return PC；7 B `jmp_buf` 保存 2 B SPX、3 B PC，并保留 2 B 结果传递区 | `ECALL`/`ERET` 的硬件帧正是 3 B | 已用 small/large 真实运行库把 SPX 设为 `0x0120`，验证 `longjmp` 恢复完整 SPX、3 B PC 和 16 位结果；四种 model/stack-auto 库均通过静态编译检查，通用递归运行测题仍会命中当前 QEMU 的 signed-dis16 缺陷 |
